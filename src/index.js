...
async function main()
{
  ...
  if (!opts.dryRun && !process.env.OPENAI_API_KEY) {
    console.warn('Missing OPENAI_API_KEY. AI features will be disabled for this run.');
  // By removing process.exit(3), the script keeps running!
  }
  ...
  if (process.env.OPENAI_API_KEY) {
    reviewer = new AIReviewer(process.env.OPENAI_API_KEY, process.env.OPENAI_BASE_URL || undefined, strModel);
  }
